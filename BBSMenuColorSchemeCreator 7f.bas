...
DIM b AS STRING ' b is used as a variable for each character in every line in the display
DIM e AS STRING ' remembers the last character ascii value in  the read in of a line of text
...
DIM colordisplay AS STRING
DIM horizontalines AS STRING
DIM verticalines AS STRING
DIM specialcharacters AS STRING 'used as as a holder for special characters
...
PRINT "%                                                                              %"
LOCATE 14, 24
PRINT "%                Alpha    Version 7.04f Release Candidate--RC6                 %"
LOCATE 15, 24
PRINT "%                                                                              %"
LOCATE 16, 24
...
CLS
PRINT
PRINT "BBS Menu Color Scheme Creator 7.0f Finished Version RC6"
PRINT "Written by Russ Campbell Updated September 6 2020"
PRINT
...
PRINT
PRINT "inet5 msg1 msg5 main1 main5 main9 file1 file5 file9 sysop8 sysop9" 'display demo files that are available
PRINT
...
answer = UCASE$(answer)
IF answer = "A" THEN display = "ansi": GOTO startit
IF answer = "B" THEN display = "wc8": GOTO startit ' select display coding by what the user types in
IF answer = "C" THEN display = "pcb": GOTO startit
IF answer = "D" THEN display = "syn": GOTO startit
IF answer = "E" THEN display = "mys": GOTO startit
GOTO displays
background:
clored$ = "None"
CLS
PRINT
PRINT "Please enter the color of the code desired for "
PRINT basesel$
...
PRINT
IF lastsel = 0 THEN colord$ = "Black"
IF lastsel = 1 THEN colord$ = "Blue"
IF lastsel = 2 THEN colord$ = "Green"
IF lastsel = 3 THEN colord$ = "Cyan"
IF lastsel = 4 THEN colord$ = "Red"
IF lastsel = 5 THEN colord$ = "Magenta"
IF lastsel = 6 THEN colord$ = "Brown"
IF lastsel = 7 THEN colord$ = "White"
IF lastsel = 8 THEN colord$ = "Flashing or Bright Black"
IF lastsel = 9 THEN colord$ = "Flashing or Bright Blue"
IF lastsel = 10 THEN colord$ = "Flashing or Bright Green"
IF lastsel = 11 THEN colord$ = "Flashing or Bright Cyan"
IF lastsel = 12 THEN colord$ = "Flashing or Bright Red"
IF lastsel = 13 THEN colord$ = "Flashing or Bright Magenta"
IF lastsel = 14 THEN colord$ = "Flashing or Bright Brown"
IF lastsel = 15 THEN colord$ = "Flashing or Bright White"
PRINT
PRINT "Last Color Selected Was "; colord$
RETURN
...
GOSUB background
PRINT
INPUT "Enter Background color for '[' and ']' symbols : ", selection1(sss)
IF selection1(sss) > 7 AND display = "syn" OR selection1(sss) > 7 AND display = "ansi" THEN GOSUB background: GOTO basecol3
IF selection1(sss) > 15 THEN sss = -1: GOSUB background: GOTO basecol3
INPUT "Enter Foreground color for '[' and ']' symbols : ", selection2(sss)
IF selection2(sss) > 7 AND display = "syn" OR selection2(sss) > 7 AND display = "ansi" THEN GOSUB background: GOTO basecol3
IF selection2(sss) > 15 THEN sss = -1: GOSUB background: GOTO basecol3
basecol:

FOR sss = 1 TO linecounter
    GOSUB background
    PRINT
    INPUT "Inside Lines Background Color Selection : ", selection1(sss) 'a background color selection
    IF selection1(sss) > 7 AND display = "syn" OR selection1(sss) > 7 AND display = "ansi" THEN selection1(sss) = -1: GOSUB background: GOTO basecol
    IF selection1(sss) > 15 THEN selection2(sss) = -1: GOSUB background: GOTO basecol
    INPUT "Inside Lines Foreground Color Selection : ", selection2(sss) 'a foreground color selection
    IF selection2(sss) > 7 AND display = "syn" OR selection2(sss) > 7 AND display = "ansi" THEN selection2(sss) = -1: GOSUB background: GOTO basecol
    IF selection2(sss) > 15 THEN selection2(sss) = -1: GOSUB background: GOTO basecol
NEXT sss
basecol2:
GOSUB background
PRINT
INPUT "Lines and corners Background Color : ", selection1(1) ' Lines Background color
IF selection1(1) > 7 AND display = "syn" OR selection1(1) > 7 AND display = "ansi" THEN GOSUB background: GOTO basecol2
IF selection1(1) > 15 THEN GOSUB background: GOTO basecol2
INPUT "Lines and corners Foreground Color : ", selection2(1) ' Lines Foreground color
IF selection2(1) > 7 AND display = "syn" OR selection2(1) > 7 AND display = "ansi" THEN GOSUB background: GOTO basecol2
IF selection2(1) > 15 THEN GOSUB background: GOTO basecol2
CLS
PRINT
INPUT "Code by [C]haracter [W]ord or [L]ine : ", answer
answer = UCASE$(answer) ' ask user if they want color display by character , word
colordisplay = answer ' or line , colors in each character , word or line
...
DO UNTIL EOF(1)
    ...
    IF display = "mys" THEN
        ...
        END IF
    END IF
    IF colordisplay = "W" THEN ' If color display is by line do the next lines of code
    ...
    IF colordisplay = "C" THEN ' If color dzisplay is by line do the next lines of code
...
PRINT
PRINT "Thank you for using BBS Menu Color Scheme Creator 7.0c RC3."
PRINT
END
pick:
RANDOMIZE TIMER
...
END IF
IF display = "wc8" AND colorchange$ = "Y" THEN
...
e = b
b$ = e
IF b$ = CHR$(91) OR b$ = CHR$(93) THEN
    flag$ = "Y1"
ELSE
END IF
IF b$ = CHR$(94) THEN b$ = corner1: flag$ = "Y2"
IF b$ = CHR$(43) THEN b$ = corner2: flag$ = "Y2"
IF b$ = CHR$(37) THEN b$ = corner3: flag$ = "Y2"
IF b$ = CHR$(126) THEN b$ = corner4: flag$ = "Y2"
IF b$ = CHR$(45) THEN
    b$ = lineconnector: flag$ = "Y2"
END IF
IF b$ = CHR$(91) THEN
    flag$ = "Y3"
END IF
...
IF b$ = CHR$(33) THEN
    ...
    flag$ = "Y2"
END IF
IF MID$(lnumber, i, 1) = CHR$(91) THEN
    flag = "Y3"
END IF

IF MID$(lnumber, i, 1) = CHR$(93) THEN
    flag$ = "Y3"
END IF

RETURN
buildit:
MID$(lnumber2$, i, 1) = b$ ' Changes the ascii value of each character in lnumber2$
c$ = b$
RETURN
